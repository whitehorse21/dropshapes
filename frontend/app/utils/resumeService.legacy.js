...
// Observables for state (legacy RxJS store - use resumeService.ts for new resume flow)
...
export const selectResumeTemplate = (templateType) => {
  if (typeof window === 'undefined') return;
  try {
    const templateData = { resumeTemplateType: templateType, selectedAt: new Date().toISOString() };
...
const generateResumeCSS = (styleData) => {
  const { fontFamily, fontSize, fontWeight, lineHeight, color, backgroundColor } = styleData || {};
  return `
    .resume-template {
    ...
    .resume-template * { ${fontFamily ? `font-family: inherit;` : ''} }
  `;
};
