...
async function handleStandardEndpoint(request, path, method) {
  ...
  if ([301, 302, 307, 308].includes(response.status)) {
    const location = response.headers.get('location');
    console.log(`Standard endpoint redirect: ${response.status} -> ${location}`);
    ...
    if (location) {
      ...
      try {
        const redirectResponse = await makeApiRequest(request, fixedLocation, method);
        return createProxyResponse(redirectResponse);
      } catch (redirectError) {
...
async function makeApiRequest(request, apiUrl, method) {
  ...
  try {
    ...
    const enhancedError = new Error(`Failed to fetch ${apiUrl}: ${fetchError.message}`);
