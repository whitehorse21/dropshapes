...
export async function GET(request, { params }) {
  ...
  const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'https://api.www.dropshapes.com';
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`Failed to fetch discussions list: ${response.status} ${response.statusText}`);
      return NextResponse.json(
        { error: 'Failed to fetch discussions list', statusCode: response.status },
    ...
    const discussion = discussions.find(d => d.id.toString() === discussionId.toString());
    
    if (!discussion) {
      console.error(`Discussion ID ${discussionId} not found in list of ${discussions.length} discussions`);
      return NextResponse.json(
