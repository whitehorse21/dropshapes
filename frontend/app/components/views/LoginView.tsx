            ...
            <div className="login-container">
                ...
                <form className="auth-form" onSubmit={handleAuth}>
                    ...
                    <button type="submit" className="btn-submit-auth" disabled={loading}>
                        {loading ? 'Processing...' : (activeTab === 'signin' ? 'Enter Dropshapes' : 'Create Account')}
                    </button>
                ...
                <div className="social-row">
                    <button type="button" className="social-btn" onClick={() => handleSocialLogin('google')} title="Google">
                        <svg viewBox="0 0 24 24"><path d="M12.48 10.92v3.28h7.84c-.24 1.84-.853 3.187-1.787 4.133-1.147 1.147-2.933 2.4-6.053 2.4-4.827 0-8.6-3.893-8.6-8.72s3.773-8.72 8.6-8.72c2.6 0 4.507 1.027 5.907 2.347l2.307-2.307C18.747 1.44 16.133 0 12.48 0 5.867 0 .533 5.333.533 12S5.867 24 12.48 24c3.44 0 6.04-1.133 7.973-3.067 1.96-1.96 2.533-4.707 2.533-6.947 0-.467-.04-.933-.093-1.373h-10.413z" /></svg>
                    </button>
                    <button type="button" className="social-btn" onClick={() => handleSocialLogin('apple')} title="Apple">
                        <svg viewBox="0 0 24 24"><path d="M17.05 20.28c-.98.95-2.05.8-3.08.35-1.09-.46-2.09-.48-3.24 0-1.44.62-2.2.44-3.06-.35C2.79 15.25 3.51 7.59 9.05 7.31c1.35.07 2.29.74 3.08.74 1.18 0 2.45-1.02 3.47-1.06.73 0 2.27.18 3.11 1.41-.03.02-1.87 1.09-1.89 4.38-.02 3.51 3.09 4.7 3.09 4.7-.01 0-.52 1.83-1.86 3.8zM12.03 7.25c-.06-3.01 2.46-4.52 2.46-4.52-.06 0-2.65-.18-4.54 2.1-1.6 1.93-1.35 4.67-1.35 4.67 2.09 0 3.5-2.23 3.43-2.25z" /></svg>
                    </button>
                    <button type="button" className="social-btn" onClick={() => handleSocialLogin('twitter')} title="X (Twitter)">
                        <svg viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" /></svg>
                    </button>
                    <button type="button" className="social-btn" onClick={() => handleSocialLogin('github')} title="GitHub">
