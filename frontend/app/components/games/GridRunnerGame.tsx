            ...
            {gameState === 'idle' && (
                <GameCard title="Grid Runner" icon={<Zap className="w-6 h-6" />}>
                    <div className="text-center space-y-6">
                        <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto bg-gradient-to-br from-blue-500/25 to-cyan-500/25 rounded-2xl flex items-center justify-center text-4xl sm:text-5xl border border-blue-400/20 shadow-lg shadow-blue-500/20">🏃‍♂️</div>
                        <p className="text-slate-300 text-base sm:text-lg leading-relaxed">
                            Solve math problems while running! Use <kbd className="px-2 py-1 bg-slate-800/80 rounded-lg border border-white/10 text-sm font-mono">←</kbd> and <kbd className="px-2 py-1 bg-slate-800/80 rounded-lg border border-white/10 text-sm font-mono">→</kbd> to pick the right lane.
                        </p>
                                ...
                                return (
                                    <div key={gate.id} className="absolute w-full flex" style={{ top: `${gate.y}px` }}>
                                        <div className="absolute -top-10 sm:-top-12 left-1/2 transform -translate-x-1/2 bg-slate-800 px-2 py-1.5 sm:px-4 sm:py-2 rounded-lg border border-slate-600 max-w-[95%]">
                                            <span className="text-white font-bold text-sm sm:text-lg">{gate.question} = ?</span>
                                        </div>
                                        {answers.map((answer, index) => (
                                            <div
                                                key={index}
                                                className={`flex-1 h-12 sm:h-16 flex items-center justify-center border-2 ${answer === gate.correctAnswer
                                                        ? 'bg-green-900/30 border-green-500'
                                                        : 'bg-red-900/30 border-red-500'
                                                    }`}
                                            >
                                                <span className="text-lg sm:text-2xl font-bold text-white">{answer}</span>
                                            </div>
                                        ))}
                                    </div>
                                );
