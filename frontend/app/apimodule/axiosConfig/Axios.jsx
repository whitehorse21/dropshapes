  ...
  (config) => {
    ...
    if (config.url) {
      ...
      if (
        ...
        // Chat message audio (no trailing slash)
  ...
  async (error) => {
    ...
    if (
      ...
      if (redirectUrl) {
        ...
        console.error(
          "CORS-causing redirect detected. Server configuration needs to be fixed.",
        );
        console.error(
          "Suggested fix: Ensure API server handles both with/without trailing slashes without redirects",
        );
      }
    }
    ...
    if (
      ...
      (error.code === "ERR_NETWORK" &&
        error.config?.url?.includes("api.www.dropshapes.com"))
    ) {
      ...
      const corsError = new Error(
        "CORS policy violation - likely due to server redirect",
      );
    ...
    if (error.response) {
      ...
      if (error.code === "ENOTFOUND" || error.code === "EAI_AGAIN") {
        console.error(
          "DNS Resolution Error: The API server domain cannot be resolved. Check your internet connection and DNS settings.",
        );
      } else if (error.code === "ECONNREFUSED") {
        console.error(
          "Connection Refused: The API server is not accepting connections. The server may be down.",
        );
      } else if (error.code === "ETIMEDOUT") {
        console.error(
          "Connection Timeout: The API server is not responding within the timeout period.",
        );
      } else if (error.code === "ERR_NETWORK") {
        console.error(
          "Network Error: General network connectivity issue. Check your internet connection.",
        );
      }
    } else {
...
if (typeof window !== "undefined") {
  ...
  const checkAPIConnection = async () => {
    ...
    try {
      ...
      console.log(
        "⚠️ API connectivity check failed (this may be normal for production)",
      );
      ...
      if (!baseURL || !baseURL.includes("api.www.dropshapes.com")) {
        ...
        try {
          ...
          console.warn(
            "Failed to auto-detect working API, continuing with current URL",
          );
        }
      }
    }
