...
export async function safeFetch(endpoint, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(
        `API request failed: ${response.status} ${response.statusText} for URL: ${url}`,
      );

      // For CORS errors, suggest using the proxy
      if (response.status === 0 || response.type === "opaqueredirect") {
        console.warn(
          "Possible CORS issue detected. Consider using the Next.js API proxy.",
        );
    ...
    if (
      ...
      console.warn(
        "Detected likely CORS issue - consider using the proxy route instead",
      );
    }
...
export async function multiStrategyFetch(endpoint, options = {}) {
  ...
  if (endpoint.includes("discussions/") && endpoint.match(/discussions\/\d+/)) {
    try {
      ...
      if (discussionId) {
        ...
        errors.push(
          `Strategy 0 failed: ${response.status} ${response.statusText}`,
        );
      }
    } catch (error) {
  ...
  if (endpoint.includes("discussions/")) {
    try {
      console.log(
        "Strategy 4: Checking alternative endpoint format (singular)",
      );
      const discussionId = endpoint.match(/discussions\/(\d+)/)?.[1];
      ...
      if (discussionId) {
        ...
        errors.push(
          `Strategy 4 failed: ${response.status} ${response.statusText}`,
        );
      } else {
  ...
  if (endpoint.includes("discussions/")) {
    try {
      ...
      if (discussionId) {
        ...
        console.log(
          `- Fetching all discussions and filtering for ID: ${discussionId}`,
        );
        ...
        if (response.ok) {
          ...
          if (Array.isArray(allDiscussions)) {
            ...
            const targetDiscussion = allDiscussions.find(
          ...
          errors.push(
            `Strategy 5 failed: ${response.status} ${response.statusText}`,
          );
        }
      } else {
