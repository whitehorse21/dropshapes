...
function InterviewTrainingContent() {
  ...
  const [answers, setAnswers] = useState<
    Array<{ question_id: string; answer_type: string; user_answer: string }>
  >([]);
  ...
  const generateQuestions = async () => {
    ...
    try {
      const res = await axiosInstance.post(
        "interview-training/questions/job-specific",
        {
  ...
  const currentAnswer =
    answers.find(
      (a) => a.question_id === currentQuestion[currentIndex]?.question_id,
    )?.user_answer ?? "";
  ...
  const setAnswerForCurrent = (value: string) => {
    ...
    setAnswers((prev) => {
      ...
      return [
        ...rest,
        { question_id: q.question_id, answer_type: "text", user_answer: value },
      ];
    });
  };
  ...
  return (
    <section
      id="view-interview-training"
      className="view-section active-view"
      aria-label="Interview Training"
    >
      <div className="interview-training-page">
        ...
        <div className="interview-training-nav">
          ...
          <Link
            href="/interview-training/performance"
            className="btn-resume"
        ...
        {!hasSession && (
          <div className="interview-training-card">
            ...
            <p className="interview-training-card-sub">
              Enter a topic and level, then generate questions. Optionally add a
        ...
        {hasSession && currentQuestion.length > 0 && (
          <div className="interview-training-card">
            <div
              className="interview-training-actions"
              style={{ marginBottom: "1rem" }}
            >
              <div>
                <h2>{selectedTopic} Interview</h2>
                <p
                  className="interview-training-card-sub"
                  style={{ marginBottom: 0 }}
                >
                  {currentQuestion.length} questions
                </p>
              </div>
              <button
                type="button"
            ...
            {!feedback ? (
              <>
                <div className="interview-training-question-block">
                  <p className="q-label">
                    Question {currentIndex + 1} of {currentQuestion.length}
                  </p>
                  <p style={{ color: "var(--text-primary)", margin: 0 }}>
                    {currentQuestion[currentIndex]?.question_text}
                  </p>
                </div>
                <div
                  className="interview-training-form-row"
                  style={{ marginBottom: "1rem" }}
                >
                  <div>
              ...
              <div>
                ...
                {feedback.evaluations?.map((evalItem, idx) => (
                  <div
                    ...
                    className="interview-training-eval-item"
                  >
                    <h4>
                      Question {idx + 1} (Score: {evalItem.score ?? 0}/100)
                    </h4>
                    ...
                    {evalItem.criteria && (
                      <div className="criteria">
                        Clarity: {evalItem.criteria.clarity} | Technical:{" "}
                        {evalItem.criteria.technical_accuracy} | Confidence:{" "}
                        {evalItem.criteria.confidence}
                      </div>
                    )}
                  </div>
                ))}
        ...
        {hasSession && currentQuestion.length === 0 && !feedback && (
          <div
            className="interview-training-card"
...
export default function InterviewTrainingPage() {
  return <InterviewTrainingContent />;
}
